...
class TaskDB(Base):
    ...
    def validate_status_transition(old_status, new_status):
        if (
            (old_status == "pending" and new_status == "in_process")
            or (old_status == "in_process" and new_status == "completed")
...
class TaskCreate(BaseModel):
    ...
    def validate_title(cls, title: str):
        if len(title) < 5:
            raise HTTPException(
                status_code=422, detail="please enter the atleast 5 character title"
            )
        if title.isnumeric():
            raise HTTPException(
                status_code=422, detail="please enter the atleast character title"
            )
        ...
        if bool(re.search(r"[^a-zA-Z0-9 ]", title)):
            raise HTTPException(
                status_code=422, detail="special character is not allowed!!"
            )
        return title
...
def apply_filter(db, queryparm, filters):
    if queryparm == "status":
        ...
        if db_task is None:
            raise HTTPException(
                status_code=422, detail="there is not values in status!!"
            )
        return db_task
    if queryparm == "priority":
        ...
        if db_task is None:
            raise HTTPException(
                status_code=422, detail="there is not values in priority!!"
            )
        return db_task
    return None
...
def paginate(db, page, limit):
    ...
    db_pages = db_task[start:end]
    return db_pages


def generate_task_message(task):
    if task.is_overdue:
        return f"This task is overdue by {-task.days_left} days"
    elif task.days_left == 0:
        return "This task is due today"
    else:
        return f"This task has {task.days_left} days left"
...
def update_by_id(task_id: int, task: TaskCreate, db: Session = Depends(get_db)):
    ...
    if not (TaskDB.validate_status_transition(db_task.status, task.status)):
        raise HTTPException(
            status_code=422, detail="status is wrong!! transsion error!!"
        )
    if task.priority == "high" and task.status == "pending":
        TaskDB.can_create_high_priority(db)
...
@app.get("/tasks/summary/{id}", response_model=TaskSummaryResponse)
